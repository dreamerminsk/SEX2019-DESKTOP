...
public class SexComApp extends Application {
  ...
  private void random() throws SQLException {
    ...
    Disposable updater =
        randoms
            ...
            .filter((User u) -> LocalDateTime.now().compareTo(u.getlastChecking().plusDays(1)) > 0)
    ...
    try {
      ...
      Disposable updater =
          Flowable.fromIterable(users)
              .filter(
                  (User u) -> LocalDateTime.now().compareTo(u.getlastChecking().plusDays(1)) > 0)
              .flatMap(user -> NetLoader.getString(user.getAbsRef()))
              .flatMap(html -> Flowable.just(Jsoup.parse(html)))
              .flatMap(UserParser::parse)
              .filter((User u) -> u.getRef() != null)
  ...
  private void settings() throws SQLException {
    UserDao userDao = (UserDao) ModelSource.getUserDAO();
    Flowable.fromIterable(userDao.queryForAll())
      ...
      if (empty) {
        ...
        userPane.setText(
            item.getName() == null
                ...
                : item.getName()
                    + " "
                    + item.getlastChecking().format(DateTimeFormatter.ISO_LOCAL_DATE_TIME));
